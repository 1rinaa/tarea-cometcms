                    ...
                    <div class="bg-white dark:bg-[#242424] rounded-2xl border border-[#E8D5A3]/20 dark:border-[#3a3a3a] p-6 sticky top-24">
                        ...
                        <div v-if="hasActiveFilters" class="mb-6">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <!-- ✅ PAÍS - Tag destacado más grande y visible -->
                        ...
                        <!-- Filtro por país - AHORA DESTACADO CON BANDEJA Y FONDO DORADO -->
                        <div class="mb-6">
                            <label class="block text-xs font-light uppercase tracking-wider text-[#55565A]/60 dark:text-[#F3E09C]/40 mb-2">
                                País
                            </label>
                            <div class="w-full px-4 py-3 bg-gradient-to-r from-[#BEA260]/15 to-[#F3E09C]/15 dark:from-[#BEA260]/10 dark:to-[#F3E09C]/5 border-2 border-[#BEA260]/40 rounded-lg text-[#1A1410] dark:text-[#F3E09C] text-sm font-medium cursor-not-allowed flex items-center gap-2 shadow-inner shadow-[#BEA260]/10">
                                <span class="flex-1">{{ countryName }}</span>
                            </div>
...
const countryName = computed(() => decodeURIComponent(route.params.id || ''));
...
const breadcrumbs = computed(() => {
    const crumbs = [
        ...
        { path: route.path, label: countryName.value }
    ];
...
// Datos filtrados del país
const countryData = computed(() => {
    if (!data.value) return [];
    return data.value.filter(item => 
        item.country?.toLowerCase() === countryName.value.toLowerCase()
    );
});

// Aplicar filtros adicionales al país
const filteredData = computed(() => {
    if (!countryData.value || countryData.value.length === 0) return [];
    
    let filtered = countryData.value;
...
const hasActiveFilters = computed(() => {
    return route.query.q || route.query.year || route.query.status;
});
