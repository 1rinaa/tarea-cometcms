<template>
    <div class="min-h-screen bg-[#F5F0E8] dark:bg-[#000000]">
        ...
        <span class="text-sm font-light text-[#55565A]/60 dark:text-[#F3E09C]/40">
            Mostrando {{ results.length }} resultados
        </span>
        <div class="flex items-center gap-2">
            <label for="sort" class="text-xs font-light text-[#55565A]/60 dark:text-[#F3E09C]/40">
                Ordenar por:
            </label>
            <select 
                id="sort"
                ...
                @change="applySort"
                class="bg-[#FAF7F2] dark:bg-[#1a1a1a] border border-[#E8D5A3]/20 dark:border-[#3a3a3a] rounded-lg px-3 py-1.5 text-sm font-light text-[#1A1410] dark:text-[#F3E09C] focus:outline-none focus:border-[#BEA260] transition-colors duration-300"
            >
            ...
            <div class="flex flex-col lg:flex-row gap-8">
                    ...
                    <div class="bg-white dark:bg-[#242424] rounded-2xl border border-[#E8D5A3]/20 dark:border-[#3a3a3a] p-6 sticky top-24">
                        ...
                        <div v-if="hasActiveFilters" class="mb-6">
                            ...
                            <div class="flex flex-wrap gap-2">
                                ...
                                <span v-if="route.query.country" 
                                      class="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-[#BEA260]/10 text-[#BEA260] text-xs font-light">
                                    {{ route.query.country }}
                                    <button @click="removeFilter('country')" class="hover:text-[#F3E09C]">
                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                        </svg>
                                    </button>
                ...
                <div class="flex-1">
                    ...
                    <!-- Grid de resultados con ResultCard -->
...
import ResultCard from '~/components/oscar/ResultCard.vue';
...
const results = computed(() => {
    ...
    switch (sort) {
        case 'year_desc':
            sorted.sort((a, b) => b.year_ceremony - a.year_ceremony);
            break;
        case 'year_asc':
            sorted.sort((a, b) => a.year_ceremony - b.year_ceremony);
            break;
        case 'title_asc':
...
const hasActiveFilters = computed(() => {
    return route.query.q || route.query.year || route.query.category || route.query.status || route.query.country;
});
...
// Aplicar filtros
const applySearch = () => {
    router.push({ query: { ...route.query, q: localSearch.value || undefined } });
};

const applyYear = () => {
    router.push({ query: { ...route.query, year: selectedYear.value || undefined } });
};
...
const applyStatus = () => {
    router.push({ query: { ...route.query, status: selectedStatus.value || undefined } });
};
...
const clearAllFilters = () => {
    ...
    selectedSort.value = 'year_desc'; 
    router.push({ query: { sort: 'year_desc' } }); 
};

// ✅ Sincronizar filtros con query params - INCLUIR selectedCountry
watch(() => route.query, () => {
    currentPage.value = 1;
    ...
    selectedSort.value = route.query.sort || 'year_desc'; 
}, { immediate: true });
